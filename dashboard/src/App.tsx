...
export default function App() {
  ...
  const load = useCallback(async () => {
    try {
      const [tl, st, dc] = await Promise.all([getTimeline(project), getStats(), getDocs(project)])
      setEvents(tl.events)
      setRunId(tl.run ?? "")
      setStats(st)
      setDocs(dc.docs)
      ...
      <header className="border-b sticky top-0 z-10 bg-background/80 backdrop-blur">
        <div className="mx-auto max-w-6xl px-6 py-3 flex items-center gap-4">
          <div className="flex items-center gap-2 font-semibold">
            ...
            <span className="text-muted-foreground font-normal text-sm">· activity</span>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <Card className="lg:col-span-2">
            <CardHeader className="pb-3">
              ...
              {runId && <div className="text-xs text-muted-foreground font-mono truncate">{runId}</div>}
            </CardHeader>
            <Separator />
