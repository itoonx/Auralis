// analyze(goal) — the usable one-shot: run the society over the repo, auto-buildGraph the findings into the
...
export async function analyze(
  ...
  deps: {
    ...
    buildGraphAll: () => Promise<number>; // auto-trigger: build the graph from the new findings
  ...
  await deps.buildGraphAll(); // auto-buildGraph (real predicates) → the graph
