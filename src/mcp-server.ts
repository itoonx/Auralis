  ...
  async ({ goal, dir, project }) => {
    ...
    try {
      ...
      const { outcome } = await runFleet("mcp", new OracleAdapter(), nodes, {
        projectDir, project: project ?? "default", maxTurns: 10, concurrency: 3, maxRetries: 1, workerPull: true,
      });
...
server.tool(
  ...
  async ({ goal, dir, accept }) => {
    ...
    try {
      ...
      const written = [...new Set(outcome.perWorker.flatMap((w) => w.explored).filter((e) => e.tool === "Write" || e.tool === "Edit").map((e) => e.target))];
      let verdict = "";
      if (accept) {
        const r = runAcceptance(projectDir, accept);
        verdict = `\n\nacceptance (${accept}): ${r.pass ? "✅ PASS" : `❌ FAIL\n${r.failLines}`}`;
      }
