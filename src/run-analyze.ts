// `pnpm analyze "<goal>"` — the real usable mode. Runs the society once over the repo, auto-builds the graph from the
// findings into the knowledge graph (real predicates by default), then answers the goal from graph-aware
// recall. Quality is on by default: AURALIS_SEMANTIC and AURALIS_BUILD_GRAPH_LLM default to 1 here (set to 0
...
import { buildGraph, extractTriplets, llmExtractTriplets } from "./graph";
...
const BUILD_GRAPH_LLM = process.env.AURALIS_BUILD_GRAPH_LLM !== "0"; // real predicates by default
...
async function main() {
  ...
  try {
    ...
    const res = await analyze(adapter, PROJECT, goal, {
      ...
      buildGraphAll: async () => {
        const runner = BUILD_GRAPH_LLM ? new ClaudeCodeRunner({ cwd: PROJECT_DIR, maxTurns: 3 }) : null;
        ...
        let edges = 0;
        for (const d of docs) edges += (await buildGraph(adapter, d.id, PROJECT, d.content, { extract })).length;
        console.log(`graphed ${docs.length} finding(s) → ${edges} edge(s)  (${BUILD_GRAPH_LLM ? "LLM predicates" : "heuristic"})`);
        return edges;
      },
