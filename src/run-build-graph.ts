// `pnpm build-graph` — build the knowledge graph: extract entity/relationship triplets from the brain's raw
// findings and store them as edges (linked by normalized entity). AURALIS_BUILD_GRAPH_LLM=1 uses Claude Code
// for real predicates (now the DEFAULT — set AURALIS_BUILD_GRAPH_LLM=0 for the free heuristic). Falls back
...
const LLM = process.env.AURALIS_BUILD_GRAPH_LLM !== "0"; // real predicates by default; =0 for heuristic
...
async function main() {
  ...
  try {
    ...
    let linked = 0;
    for (const d of docs) {
      const triplets = await buildGraph(adapter, d.id, PROJECT, d.content, { extract });
